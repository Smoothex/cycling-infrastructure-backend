...
Before map matching, the geodesic distance between the first and last valid GPS points is calculated. Rides whose origin and destination are less than **500 meters** apart (`analysis.minimum-origin-destination-distance-meters=500`) are stored with `SKIPPED` status and excluded from map matching, usage counting, and detour analysis. This prevents round trips and recreational loops with an almost zero-length shortest path from producing disproportionate preference signals.

### 2. Shortest Path Computation

GraphHopper uses a *profile* to define which network edges a mode of transport may use and how traversable edges are weighted. Two separately named profiles are used so that the reconstruction of the observed route and the construction of the comparison baseline have explicit, independent roles:

| Profile | Purpose | Bicycle access | Speed | Infrastructure priority | Distance influence | Turn penalty | Routing mode |
|---|---|---|---:|---:|---:|---:|---|
| `bike_match_neutral` | Connect plausible candidates during GPS map matching | Directional `bike_access`; inaccessible and private edges blocked | 20 km/h | 1 for accessible edges; 0 otherwise | 0 s/km | none | flexible |
| `bike_shortest` | Construct the origin--destination shortest path | Directional `bike_access`; inaccessible and private edges blocked | 20 km/h | 1 for accessible edges; 0 otherwise | 0 s/km | none | Contraction Hierarchy |

Both profiles deliberately use the same neutral edge weights. On every accessible edge, the modelled speed is the constant 20 km/h and the priority multiplier is 1. GraphHopper's finite routing cost is therefore a constant multiple of the edge length; setting distance influence to 0 adds no second cost term. Minimizing this cost is consequently equivalent to minimizing physical route length. The resulting baseline is an exact minimum-distance path within GraphHopper's bicycle-accessible network, rather than merely a distance-prioritized route.

The `bike_access` value is directional, so applicable one-way restrictions are respected. Edges that prohibit cycling and edges marked as private are excluded. No cycleway, road-class, surface, speed-limit, or other infrastructure preference is applied. This prevents the routing profiles from assuming that cyclists prefer the same infrastructure characteristics that the subsequent analysis is intended to measure.
